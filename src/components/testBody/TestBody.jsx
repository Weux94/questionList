...
export default function TestBody() {
  ...
  const [visible, setVisible] = useState(true); // Изначально показываем вопрос

  const ANIMATION_DURATION = 500; // длительность анимации в мс (должна совпадать с CSS)
  ...
  const handleClick = (e) => {
    ...
    // Сначала начинаем анимацию скрытия вопроса
    setVisible(false);

    setTimeout(() => {
      if (index < questions.length - 1) {
        setIndex(prev => prev + 1);
        setVisible(true); // показываем следующий вопрос
      } else {
        setShowResult(true);
      }
    }, ANIMATION_DURATION);
